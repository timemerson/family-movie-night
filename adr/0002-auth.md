...
- Account deletion (US-23) requires deleting both the Cognito user and the DynamoDB User record. For independent users with managed members: all managed members owned by that user (`parent_user_id`) must also be deleted (cascade), and their votes/ratings anonymized to "[Deleted User]".
- **Managed member identity:** Managed members have no Cognito account. Their `user_id` uses the `managed_<uuid>` namespace. Actions on behalf of a managed member are authenticated via the parent's Cognito token plus the `X-Acting-As-Member` HTTP header, validated by auth middleware (see `docs/architecture/api.md`).
